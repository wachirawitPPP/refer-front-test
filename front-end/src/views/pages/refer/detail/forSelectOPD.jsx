...
import { Checkbox, Chip, FormControlLabel, Grid, TablePagination, Tooltip, IconButton } from '@mui/material'
import AddUserComponenet from '@/app/[lang]/(dashboard)/pages/referTable2/addModalComponent'
// Third-party Imports
...
const parseDate = date => {
  const dateFormat = String(date).slice(0, 10)
  return dateFormat
}
...
const Columns = ({ columnHelper, locale, defaultCheck, page, onPage, handleChange, handleOpdViewClick }) =>
  useMemo(
    () => [
      {
        header: '#',
        ...
        header: ({ table }) => (
          <>
            ...
            {/* <FormControlLabel
                ...
                /> */}
            {Object.keys(onPage).map(
              key =>
                page == key && (
                  <div key={key}>
                    <Checkbox checked={onPage[key]} onChange={() => defaultCheck(key)} />
                    Page {key}
                  </div>
                )
            )}
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-2'>
            <Tooltip title='แก้ไขข้อมูล'>
              <IconButton
                onClick={() => {
                  handleOpdViewClick(row.original.opds)
                }}
              >
    ...
    [columnHelper, locale, page, handleChange, handleOpdViewClick]
  )

export default function ForSelectOPD({ selectOPD, setSelectOPD, selectedUser }) {
  ...
  const [itemOpd, setItemOpd] = useState(null)
  ...
  const getDataOpds = async () => {
    try {
      ...
      const fetchedData = res.data.opd;
      console.log("Fetched Data:", fetchedData);

      // ตั้งค่า state ด้วยข้อมูลที่ดึงมา
      setDataOpd(fetchedData);

      // ส่งค่ากลับหากต้องการใช้งานต่อ
      return fetchedData;
    } catch (error) {
  ...
  const columns = Columns({ columnHelper, locale, defaultCheck, page, onPage, handleChange, handleOpdViewClick })
  ...
  return (
    <>
      <Grid container spacing={4}>
        <Grid item xs={12} style={{ overflow: 'auto' }}>
          ...
          {selectedData.map(item => (
            <Chip key={item.id} color='success' className='mr-2' label={item.opd_code} />
          ))}
          <br />
          <br />
          <TablePagination
      ...
      <AddUserComponenet open={openModal} dataEditOpd={itemOpd} onClose={handleCloseModal} />
    </>
  )
}
