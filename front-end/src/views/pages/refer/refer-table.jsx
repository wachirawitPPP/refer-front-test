...
const Modal = ({ isOpen, onClose, content, onSave, referid }) => {
  ...
  const handleSaveClick = async () => {
    ...
    //const dataEdit = { note: editableContent };
...
const ReciveModal = ({ isOpen, onClose, content, onSave, referid }) => {
  const [isEditing, setIsEditing] = useState(false)
  const [editableContent, setEditableContent] = useState(content)
  const { data: session } = useSession()
  const handleEditClick = () => {
...
const DoctorNotesCell = ({ data, index }) => {
  //const [isModalOpen, setIsModalOpen] = useState(false);

  const [isModalOpenRecive, setIsModalOpenRecive] = useState(false)
  ...
  const toggleModalRecive = () => {
    setIsModalOpenRecive(!isModalOpenRecive)
  }
  ...
  return (
    <>
      <div className='flex items-center gap-4 p-2'>
        ...
        <ReciveModal
          isOpen={isModalOpenRecive}
          onClose={toggleModalRecive}
          content={safeNote || 'No content available'}
          maxWidth='lg'
          referid={referid}
          index={index}
  ...
  return (
    <>
        ...
        <>
          ...
          <Tooltip title='ดูรูปภาพ'>
            <IconButton onClick={handleViewImageClick}>
              <i className='tabler-photo text-[25px] text-primary hover:text-secondary cursor-pointer' />
            </IconButton>
          </Tooltip>
          {/* <Chip
            variant='tonal'
                  ...
                  <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 p-2'>
                    {pdffiles.map((file, index) => (
                      ...
                      >
                        ...
                        <div
                          className='w-[140px] h-[180px] relative rounded-lg overflow-hidden 
                        ...
                        <div className='mt-2 w-[140px]'>
                          <Typography variant='caption' className='text-gray-600 block text-center truncate px-2'>
                            {file.alt.slice(0, 20)}
                          </Typography>
                        </div>
                      </div>
      ...
      <Viewer
        ...
        //onClose={() => setViewerVisible(false)}  // ปิด viewer
                  ...
                  <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 p-2'>
                    {pdffiles.map((file, index) => (
                      ...
                      >
                        {/* กำหนดขนาดคงที่สำหรับ container */}
                        <div
                          className='w-[120px] h-[140px] relative rounded-lg overflow-hidden 
                                    border border-gray-200 bg-gray-50 flex items-center justify-center'
                        >
                          <div className='w-full h-full flex items-center justify-center'>
                            <div className='w-24 h-24 relative flex items-center justify-center'>
...
const Columns = ({ columnHelper, locale, handleEdit, UploadImage }) =>
  useMemo(
    () => [
      {
        header: 'NO',
        ...
        cell: ({ row }) => (
          <div className='flex items-start gap-4'>
            <div className='flex flex-col'>
              <a href={`refer-detail/${row.original.id}?id_card=${row.original.id_card}&refer=true`}>
...

      {
        ...
        cell: ({ row, index }) => <NotesCell data={row} index={index} />
      },
      {
      ...
      },
      {
        ...
        cell: ({ row, index }) => <DoctorNotesCell data={row} index={index} />
      },
...
const ReferTable = ({ tableData, onUpdate }) => {
  ...
  const filteredData = useMemo(() => {
    return tableData?.filter(
      user =>
        user.Hospital.id === session?.user?.hospitalId &&
        (user.status === '2' || user.status === '1' || user.status === '6')
    )
  }, [tableData])
  useEffect(() => {
  ...
  const columns = Columns({ columnHelper, locale, handleEdit, UploadImage })
  ...
  return (
    <>
      ...
      <div className='overflow-x-auto overflow-y-auto '>
        <div className='overflow-x-auto'>
          <table className={classnames(tableStyles.table, 'w-full text-sm')}>
            <thead className='bg-grey z-30 text-sm'>
                    ...
                    >
                      {header.isPlaceholder ? null : (
                        ...
                        >
                          ...
                          }[header.column.getIsSorted()] ?? <i className='tabler-chevron-down text-xl text-gray-400' />}
                        </div>
                      )}
                    </th>
                  ))}
      ...
      {/* <StepperCustomHorizontal isEdit={true}
        open={editDialog}
        onClose={handleClose}
